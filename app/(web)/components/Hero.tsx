...
const Hero = () => {
		...
		return (
			<div className={showMenu ? "bg-black/90 opacity-60 blur-sm transition-all duration-100" : "relative h-[calc(100vh-10rem)] w-full overflow-hidden "}>
					<div className="relative z-10 h-full   flex  items-start justify-center px-4 text-center  md:pt-48 lg:pt-88">
							<div className=" w-3/4 sm:w-2/3 h-160 flex flex-col items-center justify-center rounded-xl ">
									...
									<motion.div
										...
										className="absolute bottom-0 left-0 right-0 flex flex-wrap justify-center gap-4 sm:gap-6 lg:gap-8 px-4 sm:px-8"
									>
											{AVAILABILITY.map((a, index) => (
													<motion.div
															...
															className="group relative bg-linear-to-br from-gray-900/90 to-black/90 backdrop-blur-md px-6 sm:px-8 py-5 sm:py-6 rounded-2xl border border-gray-700/50 hover:border-emerald-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/20 hover:scale-105 cursor-default"
													>
															{/* Glow effect on hover */}
															<div className="absolute inset-0 bg-linear-to-br from-emerald-500/10 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
															
															<div className="relative flex items-center gap-3 sm:gap-4">
																	{/* Number with gradient */}
																	<span className="text-3xl sm:text-4xl font-bold bg-linear-to-br from-emerald-400 to-green-500 bg-clip-text text-transparent">
																			{a.num}
																	</span>
																	
																	{/* Divider */}
																	<div className="h-8 w-px bg-linear-to-b from-transparent via-gray-600 to-transparent"></div>
